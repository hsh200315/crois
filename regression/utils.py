...
class FeatureExtractor:
    def calculate_features(self, data):
        ...
        Z10 = np.sum(
            [(f**2) * p for f, p in enumerate(np.abs(np.fft.fft(data)) ** 2)]
        )  # (MSF)
        Z11 = np.mean(np.abs(np.fft.fft(data)) ** 2)  # (MPS)
        Z12 = np.sum(
            [f * p for f, p in enumerate(np.abs(np.fft.fft(data)) ** 2)]
        ) / np.sum(
            np.abs(np.fft.fft(data)) ** 2
        )  # (FC)
    ...
    def _load_data(self):
        ...
        for directory in self.directories:
                            ...
                            for col in csv_data.columns:
                                if csv_data[col].dtype in [
                                    ...
                                    instance_features.extend(
                                        self.calculate_features(csv_data[col].values)
                                    )
            ...
            label_file = os.path.join(
                directory, f"{os.path.basename(directory.strip('/'))}_all_labels.csv"
            )
            if os.path.exists(label_file):
                label_df = pd.read_csv(label_file)
                label_column = [col for col in label_df.columns if "Tool Wear" in col][
                    0
                ]  # 'Tool Wear'가 포함된 컬럼명 찾기
        ...
        if len(all_data) != len(all_labels):
            raise ValueError(
                "Data and labels length mismatch. Check your directories and label files."
            )
